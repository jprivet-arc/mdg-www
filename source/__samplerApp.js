enyo.kind({
    ...
    navTap: function(inSender, inEvent) {
        ...
        if (!sample.samples && !sample.path) {
            this.$.sampleContent.createComponent({
                content:"Sorry, no sample yet for \"" + sample.name + "\"."
                });
    ...
    renderSample: function(sample) {
        ...
        var instance = this.$.sampleContent.createComponent({
            kind:this.kind,
            name: enyo.uncap(kind)
            });
        ...
        if ((this.externalURL.indexOf("http") !== 0) || (this.externalURL.indexOf(window.location.origin) === 0)) {
            new enyo.Ajax({
                url: enyo.path.rewrite(sample.path + ".js"),
                handleAs:"text"
            })
            ...
            .go();
            new enyo.Ajax({
                url: enyo.path.rewrite(path + (sample.css || kind) + ".css"),
                handleAs:"text"
            })
            .response(this, function(inSender, inSource) {
...

    {
...

        {
...

            {
                ...
                {
                    content:"Toggle RTL (off)",
                    action:"toggleRTL"
                },

                {
                    content:"Toggle Non-Latin (off)",
                    action:"toggleNonLatin"
                },
...
enyo.kind({
    ...
    setupKnownIssues: function(inSender, inEvent) {
        ...
        item.$.issueKey.setAttributes({
            href:"https://enyojs.atlassian.net/browse/" + issue.key,
            target:"_blank"
        });
